...
async function ensureWirtschaftspruefer(req) {
...
exportRouter.get("/audit-package", async (req, res) => {
    try {
        ...
        if (!tenantId || !from || !to)
            return res.status(400).json({ ok: false, error: "tenantId, from, to zorunludur" });
