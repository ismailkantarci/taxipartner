...
ousRouter.get("/", permissionGuard(["tp.ou.read"]), scopeGuard(), async (req, res) => {
...
ousRouter.post("/", permissionGuard(["tp.ou.create"]), scopeGuard(), async (req, res) => {
    const tenantId = req.headers["x-tenant-id"] ||
        req.body?.tenantId;
    if (!tenantId) {
...
ousRouter.put("/:id", permissionGuard(["tp.ou.create"]), scopeGuard(), async (req, res) => {
    const tenantId = req.headers["x-tenant-id"] ||
        req.body?.tenantId ||
        (typeof req.query?.tenantId === "string"
            ? req.query.tenantId
    ...
    try {
        ...
        res
            .status(400)
            .json({ ok: false, error: error.message || "OU güncelleme hatası" });
    }
});
ousRouter.delete("/:id", permissionGuard(["tp.ou.create"]), scopeGuard(), async (req, res) => {
    try {
        ...
        res
            .status(400)
            .json({ ok: false, error: error.message || "OU silme hatası" });
    }
});
