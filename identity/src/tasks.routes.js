...
function mustParseJson(name, source) {
    if (!source)
        return {};
    try {
        return JSON.parse(source);
...
tasksRouter.put('/:id', async (req, res) => {
    try {
        ...
        if (channels !== undefined)
            data.channels = safeStringify(channels);
        if (payload !== undefined)
            data.payload = safeStringify(payload, 'null');
...
tasksRouter.post('/:id/run', async (req, res) => {
    try {
        ...
        const channels = mustParseJson('channels', task.channels);
        const payload = mustParseJson('payload', task.payload);
