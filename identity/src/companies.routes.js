...
const PERMIT_STATUS_MAP = new Map([
    ["active", "Active"],
    ["ruhend", "Ruhend"],
    ["gelöscht", "Gelöscht"],
    ["geloescht", "Gelöscht"]
...
async function ensureCompany(id, tenantId) {
    return prisma.company.findFirst({ where: { companyId: id, tenantId } });
}
function normalizePermitStatus(value) {
...
function parseIntOrNull(value, field) {
    if (value === null || typeof value === "undefined" || value === "") {
        return null;
    }
...
function activeAuthorizationCount(permitId) {
    return prisma.vehicleAuthorization.count({ where: { permitId, revokedOn: null } });
}
companiesRouter.get("/", permissionGuard(["tp.company.read"]), scopeGuard(), async (req, res) => {
...
companiesRouter.post("/", permissionGuard(["tp.company.create"]), scopeGuard(), async (req, res) => {
    const tenantId = tenantFrom(req);
...
companiesRouter.get("/:id", permissionGuard(["tp.company.read"]), async (req, res) => {
    ...
    const company = await prisma.company.findFirst({
        where: { companyId: req.companyId || req.params.id, tenantId },
        include: {
            officers: true,
            shareholders: true,
            documents: true,
            ...
            permitEvents: { orderBy: { eventDate: "desc" } }
        }
    });
    if (!company) {
...
companiesRouter.put("/:id", permissionGuard(["tp.company.update"]), scopeGuard(), async (req, res) => {
    const tenantId = tenantFrom(req) || "";
    if (!tenantId) {
        res.status(400).json({ ok: false, error: "tenantId zorunlu" });
        return;
    ...
    const company = await prisma.company.update({
        ...
        data: { legalName, address, status }
    });
    res.json({ ok: true, company });
});
companiesRouter.delete("/:id", permissionGuard(["tp.company.delete"]), scopeGuard(), async (req, res) => {
    const tenantId = tenantFrom(req) || "";
    if (!tenantId) {
        res.status(400).json({ ok: false, error: "tenantId zorunlu" });
        return;
...
companiesRouter.post("/:id/officers", permissionGuard(["tp.company.update"]), scopeGuard(), async (req, res) => {
    const tenantId = tenantFrom(req) || "";
    if (!tenantId) {
        res.status(400).json({ ok: false, error: "tenantId zorunlu" });
        return;
...
companiesRouter.delete("/:id/officers/:officerId", permissionGuard(["tp.company.update"]), scopeGuard(), async (req, res) => {
    const tenantId = tenantFrom(req) || "";
    if (!tenantId) {
        res.status(400).json({ ok: false, error: "tenantId zorunlu" });
        return;
...
companiesRouter.post("/:id/shareholders", permissionGuard(["tp.company.update"]), scopeGuard(), async (req, res) => {
    const tenantId = tenantFrom(req) || "";
    if (!tenantId) {
        res.status(400).json({ ok: false, error: "tenantId zorunlu" });
        return;
...
companiesRouter.delete("/:id/shareholders/:shareholderId", permissionGuard(["tp.company.update"]), scopeGuard(), async (req, res) => {
    const tenantId = tenantFrom(req) || "";
    if (!tenantId) {
        res.status(400).json({ ok: false, error: "tenantId zorunlu" });
        return;
...
companiesRouter.post("/:id/documents", permissionGuard(["tp.company.update"]), scopeGuard(), async (req, res) => {
    const tenantId = tenantFrom(req) || "";
    if (!tenantId) {
        res.status(400).json({ ok: false, error: "tenantId zorunlu" });
        return;
...
companiesRouter.delete("/:id/documents/:documentId", permissionGuard(["tp.company.update"]), scopeGuard(), async (req, res) => {
    const tenantId = tenantFrom(req) || "";
    if (!tenantId) {
        res.status(400).json({ ok: false, error: "tenantId zorunlu" });
        return;
...
companiesRouter.get("/:id/permits", permissionGuard(["tp.company.read"]), scopeGuard(), async (req, res) => {
    const tenantId = tenantFrom(req) || "";
    if (!tenantId) {
        res.status(400).json({ ok: false, error: "tenantId zorunlu" });
        return;
    ...
    const [permits, events] = await Promise.all([
        ...
        prisma.companyPermitEvent.findMany({ where: { companyId: company.companyId }, orderBy: { eventDate: "desc" } })
    ]);
    res.json({ ok: true, permits, events });
});
companiesRouter.post("/:id/permits", permissionGuard(["tp.company.update"]), scopeGuard(), async (req, res) => {
    const tenantId = tenantFrom(req) || "";
    if (!tenantId) {
        res.status(400).json({ ok: false, error: "tenantId zorunlu" });
        return;
    ...
    const { companyId, permitType, issuingAuthority, referenceNo, permitRegisteredOn, effectiveFrom, validUntil, capacityPkw, vehicleScoped, status } = req.body || {};
...
companiesRouter.put("/:id/permits/:permitId", permissionGuard(["tp.company.update"]), scopeGuard(), async (req, res) => {
    const tenantId = tenantFrom(req) || "";
    if (!tenantId) {
        res.status(400).json({ ok: false, error: "tenantId zorunlu" });
        return;
    ...
    const { companyId, status, permitRegisteredOn, effectiveFrom, validUntil, capacityPkw, vehicleScoped, ...rest } = req.body || {};
    if (companyId && companyId !== company.companyId) {
        res.status(400).json({ ok: false, error: "companyId rota ile eşleşmeli" });
        return;
    }
    const permit = await prisma.companyPermit.findFirst({ where: { id: req.params.permitId, companyId: company.companyId } });
    if (!permit) {
        res.status(404).json({ ok: false, error: "İzin bulunamadı" });
        return;
    }
    const data = {};
    if (typeof rest.permitType === "string")
        data.permitType = rest.permitType.trim();
    if (typeof rest.issuingAuthority !== "undefined")
        data.issuingAuthority = rest.issuingAuthority === null || rest.issuingAuthority === "" ? null : String(rest.issuingAuthority).trim();
    if (typeof rest.referenceNo !== "undefined")
        data.referenceNo = rest.referenceNo === null || rest.referenceNo === "" ? null : String(rest.referenceNo).trim();
...
companiesRouter.get("/:id/permits/:permitId/vehicles", permissionGuard(["tp.company.read"]), scopeGuard(), async (req, res) => {
    const tenantId = tenantFrom(req) || "";
    if (!tenantId) {
        res.status(400).json({ ok: false, error: "tenantId zorunlu" });
        return;
...
companiesRouter.post("/:id/permits/:permitId/vehicles", permissionGuard(["tp.company.update"]), scopeGuard(), async (req, res) => {
    const tenantId = tenantFrom(req) || "";
    if (!tenantId) {
        res.status(400).json({ ok: false, error: "tenantId zorunlu" });
        return;
    ...
    if (vehicleId) {
        const vehicle = await prisma.vehicle.findFirst({ where: { vehicleId, tenantId, companyId: company.companyId } });
        if (!vehicle) {
    ...
    const normalizedVin = normalizeVin(vin ?? (linkedVehicle === null || linkedVehicle === void 0 ? void 0 : linkedVehicle.vin) ?? "");
    ...
    try {
        ...
        res.status(400).json({ ok: false, error: (error === null || error === void 0 ? void 0 : error.message) || String(error) });
        return;
    }
    ...
    try {
        const authorization = await prisma.vehicleAuthorization.create({
            data: {
                permitId: permit.id,
                vehicleId: (linkedVehicle === null || linkedVehicle === void 0 ? void 0 : linkedVehicle.vehicleId) ?? null,
                vin: normalizedVin,
...
companiesRouter.post("/:id/permits/:permitId/vehicles/:authorizationId/revoke", permissionGuard(["tp.company.update"]), scopeGuard(), async (req, res) => {
    const tenantId = tenantFrom(req) || "";
    if (!tenantId) {
        res.status(400).json({ ok: false, error: "tenantId zorunlu" });
        return;
    ...
    const authorization = await prisma.vehicleAuthorization.findFirst({ where: { id: req.params.authorizationId, permitId: permit.id } });
    if (!authorization) {
...
companiesRouter.post("/:id/permits/:permitId/events", permissionGuard(["tp.company.update"]), scopeGuard(), async (req, res) => {
    const tenantId = tenantFrom(req) || "";
    if (!tenantId) {
        res.status(400).json({ ok: false, error: "tenantId zorunlu" });
        return;
...
companiesRouter.get("/:id/permits/:permitId/events", permissionGuard(["tp.company.read"]), scopeGuard(), async (req, res) => {
    const tenantId = tenantFrom(req) || "";
    if (!tenantId) {
        res.status(400).json({ ok: false, error: "tenantId zorunlu" });
        return;
