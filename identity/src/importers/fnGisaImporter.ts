...
export type FirmenbuchIdentity = {
...
export type FirmenbuchIdentifier = {
  type: string;
  value: string;
...
export type FirmenbuchRecord = {
  fn: string;
  legalName: string;
  legalForm?: string | null;
  seatAddress?: string | null;
  identities?: FirmenbuchIdentity[];
  identifiers?: FirmenbuchIdentifier[];
  attachments?: AttachmentInput[];
};

export type PermitEventInput = {
  type: string;
  date: string | Date;
...
export type PermitInput = {
  ...
  capacityPkw?: number | string | null;
  vehicleScoped?: boolean;
  status?: string | null;
  events?: PermitEventInput[];
  attachments?: AttachmentInput[];
};

export type GisaRecord = {
  gisa: string;
  fn: string;
  legalName?: string | null;
  address: string;
...
export type ImportReport = ImportStats & {
  dryRun: boolean;
};

const DRY_RUN_ABORT = Symbol("IMPORT_DRY_RUN_ABORT");

export async function importOfficialRegisters(request: ImportRequest, client: PrismaClient = prisma): Promise<ImportReport> {
...
async function resolveBundle(request: ImportRequest): Promise<ImportBundle> {
  const merged: ImportBundle = { firmenbuch: [], gisa: [] };
  if (request?.bundle) {
...
async function loadBundleFromSource(source: string): Promise<ImportBundle> {
...
async function tryStat(target: string) {
  try {
...
function normalizeBundle(input: any): ImportBundle {
  const result: ImportBundle = {};
...
function convertTenantStructure(rawTenants: any[]): ImportBundle {
  const firmenbuch: FirmenbuchRecord[] = [];
  const gisa: GisaRecord[] = [];

  for (const tenant of rawTenants) {
    const tenantId = pickString(tenant, ["tenantId", "fn", "id"]);
    if (!tenantId) continue;

    const firmenbuchRecord: FirmenbuchRecord = {
      fn: tenantId,
    ...
    for (const company of companies) {
      ...
      const companyRecord: GisaRecord = {
        gisa: companyId,
        fn: tenantId,
        legalName: pickString(company, ["legalName", "name"]) ?? firmenbuchRecord.legalName,
        address:
          pickString(company, ["address", "seatAddress", "seat_address"]) ??
          firmenbuchRecord.seatAddress ?? "",
...
async function upsertTenant(tx: PrismaLike, record: FirmenbuchRecord, stats: ImportStats) {
  const tenantId = record.fn?.trim();
  if (!tenantId) {
...
async function syncTenantIdentifiers(tx: PrismaLike, tenantId: string, identifiers: FirmenbuchIdentifier[]) {
...
async function syncTenantIdentityHistory(
  tx: PrismaLike,
  tenantId: string,
  record: FirmenbuchRecord,
  legalName: string,
  legalForm: string | null,
  seatAddress: string | null
) {
...
function convertIdentifiers(raw: any): FirmenbuchIdentifier[] | undefined {
  if (!Array.isArray(raw)) {
    return undefined;
  }
  const identifiers: FirmenbuchIdentifier[] = [];
  for (const entry of raw) {
    ...
    } as FirmenbuchIdentifier & { primaryFlag?: boolean };
...
function convertAttachments(raw: any): AttachmentInput[] | undefined {
  if (!Array.isArray(raw)) {
    return undefined;
  }
...
function convertPermits(raw: any): PermitInput[] | undefined {
  if (!Array.isArray(raw)) {
    return undefined;
  }
  const permits: PermitInput[] = [];
  for (const entry of raw) {
...
function convertPermitEvents(raw: any): PermitEventInput[] | undefined {
  if (!Array.isArray(raw)) {
    return undefined;
  }
  const events: PermitEventInput[] = [];
  for (const entry of raw) {
...
function pickString(record: any, keys: string[]): string | undefined {
...
function pickNumber(record: any, keys: string[]): number | null | undefined {
  for (const key of keys) {
    const value = record?.[key];
...
function pickBoolean(record: any, keys: string[]): boolean | undefined {
  for (const key of keys) {
    const value = record?.[key];
...
async function upsertCompany(tx: PrismaLike, tenantId: string, record: GisaRecord, stats: ImportStats) {
...
async function syncPermitEvents(
  tx: PrismaLike,
  permitId: string,
  companyId: string,
  events: PermitEventInput[],
