...
inviteRouter.post('/create', async (req, res) => {
  try {
    ...
    const { previewUrl } = await sendInviteMail(
      email,
      'TAXIPartner – Einladung / Invite',
      `<p>Merhaba,</p><p>Hesabını oluşturmak için bağlantıya tıkla:</p><p><a href="${acceptUrl}">${acceptUrl}</a></p>`
    );
