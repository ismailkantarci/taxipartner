...
import type { LocalePrefs, TenantSettings, Theme, UserSettings } from './types';
import {
...
type SettingsSnapshot = Readonly<SettingsState>;

const USER_STORAGE_KEY = 'tp-admin@settings:user';
const TENANT_STORAGE_KEY = 'tp-admin@settings:tenant';

let adapter: SettingsAdapter = createMemorySettingsAdapter();
...
let storedUsers =
  storage.get<Record<string, UserSettings>>(USER_STORAGE_KEY, {} as Record<string, UserSettings>) ??
  {};
let storedTenants =
  storage.get<Record<string, TenantSettings>>(TENANT_STORAGE_KEY, {} as Record<string, TenantSettings>) ??
  {};

const subscribers = new Set<() => void>();
let mediaQuery: MediaQueryList | null = null;
let lastAppliedTheme: Theme | 'dark' | 'light' | null = null;
...
const LEGACY_LANG_MAP: Record<LocalePrefs['locale'], { legacy: string; appState: string }> = {
...
const mapToLegacyLanguage = (locale: LocalePrefs['locale']) =>
  LEGACY_LANG_MAP[locale] ?? { legacy: 'en', appState: locale };

const applyLocale = (locale: LocalePrefs) => {
  ...
  const globalAppState = (globalThis as any)?.AppState;
...
const updateState = (updater: (prev: SettingsState) => SettingsState) => {
  state = updater(state);
...
const mergePreferences = (
  tenantPrefs: UserSettings['preferences'] | undefined,
  userPrefs: UserSettings['preferences'] | undefined
) => ({
...
const mergeLocale = (tenant: TenantSettings | null, user: UserSettings | null): LocalePrefs => ({
  ...SYSTEM_DEFAULT_LOCALE,
...
const mergeTheme = (tenant: TenantSettings | null, user: UserSettings | null): Theme =>
  user?.theme ?? tenant?.defaults?.theme ?? SYSTEM_DEFAULT_USER_SETTINGS.theme;
...
export const getEffectiveSettings = (snapshot: SettingsSnapshot = state): UserSettings => ({
...
export const getSettingsSnapshot = (): SettingsSnapshot => state;
...
export const registerSettingsAdapter = (nextAdapter: SettingsAdapter) => {
  adapter = nextAdapter;
};
...
export const loadSettings = async (options: { userId: string; tenantId?: string | null }) => {
  ...
  try {
    const [userSettings, tenantSettings] = await Promise.all([
      ...
      tenantId ? adapter.getTenantSettings(tenantId) : Promise.resolve<TenantSettings | null>(null)
    ]);
...
export const saveUserSettings = async (changes: Partial<UserSettings>) => {
  ensureInitialized();
...
export const saveTenantSettings = async (changes: Partial<TenantSettings>) => {
  ensureInitialized();
