...
import type { AuditAdapter, AuditEvent, AuditListQuery, AuditListResult } from './types';
...
const matchesTerm = (event: AuditEvent, query?: string) => {
...
const matchesUser = (event: AuditEvent, user?: string) => {
  if (!user) return true;
  return event.actor.id === user || event.actor.email === user;
};

const matchesAction = (event: AuditEvent, action?: string) => {
  if (!action) return true;
  return event.action === action;
};

const matchesWindow = (event: AuditEvent, from?: string, to?: string) => {
...
const sortEvents = (collection: AuditEvent[], sort?: string, order: 'asc' | 'desc' = 'desc') => {
  ...
  sorted.sort((a, b) => {
    ...
    const av = aRecord[sort];
    const bv = bRecord[sort];
    return order === 'asc'
...
export const appendAuditEvent = (partial: Omit<AuditEvent, 'id'> & { id?: string }) => {
