...
import type { ComplianceListQuery, ComplianceListResult, ComplianceRecord, ComplianceStatus, CreateComplianceInput, UpdateComplianceInput } from './types';

const SAMPLE_DATA: ComplianceRecord[] = [
  {
    id: 'CP-001',
...
const matchesSearch = (record: ComplianceRecord, term?: string) => {
  if (!term) return true;
...
const matchesStatus = (record: ComplianceRecord, statuses?: ComplianceStatus[]) => {
  if (!statuses || statuses.length === 0) return true;
  return statuses.includes(record.status);
};

const sortRecords = (records: ComplianceRecord[], sort?: string, order: 'asc' | 'desc' = 'asc') => {
...
const paginate = (records: ComplianceRecord[], page = 0, pageSize = records.length) => {
...
export const createMemoryComplianceAdapter = () => {
  ...
  const list = async (query: ComplianceListQuery): Promise<ComplianceListResult> => {
    await delay(120);
  ...
  const create = async (input: CreateComplianceInput) => {
    await delay(140);
  ...
  const update = async (id: string, input: UpdateComplianceInput) => {
    await delay(140);
  ...
  return {
    list: (query: ComplianceListQuery) =>
