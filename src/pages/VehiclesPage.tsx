...
const VehiclesContent: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const mount = async () => {
      ...
      setStatus('loading');
      try {
        const mod = await import('../../frontend/vehicles/page.ts');
        if (!mounted) return;
        const result = await mod.mountVehiclesPage(containerRef.current);
        if (!mounted) return;
        cleanup = typeof result === 'function' ? result : undefined;
        setStatus('ready');
      } catch (err) {
        if (!mounted) return;
        console.error('[vehicles] legacy mount failed', err);
    ...
    return () => {
      ...
      if (cleanup) {
        try {
          ...
          console.warn('[vehicles] cleanup failed', err);
        }
      }
  ...
  return (
    <section className="flex flex-1 flex-col gap-4">
      <header>
        <h1 className="text-xl font-semibold text-slate-900 dark:text-slate-100">Vehicles</h1>
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Tenant bazlı araç kayıtları legacy modül üzerinden yönetiliyor.
        </p>
      </header>
      {status === 'error' ? (
